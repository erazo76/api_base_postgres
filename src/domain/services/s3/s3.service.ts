...
import { S3, ListObjectsCommandInput, PutObjectCommandOutput, PutObjectCommandInput, GetObjectCommandInput } from '@aws-sdk/client-s3';
...
export class S3Service extends S3 implements IS3Service {
    ...
    private streamToString = (stream): Promise<string> => new Promise((resolve, reject) => {
        const chunks = [];
    ...
    public async uploadFile(data: string, fileName: string, path: string, bucket: string): Promise<PutObjectCommandOutput> {
        try {
